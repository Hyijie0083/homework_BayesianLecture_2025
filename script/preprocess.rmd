...
saveRDS(models_accuracy_bayes, "../results/models_accuracy_bayes_complete_pooling.rds")
...
# Model4ACC
models_analysis_2 <- list()
model_acc <- brms::brm(
  probe_accuracy ~ 1 + cue_dimensionality,
  data = data_behavior_full,
  family = bernoulli(link = "logit"),
  iter = 2000,
  warmup = 1000,
  chains = 4,
  ...
  seed = 42)
# name
model_name <- paste0("model_acc_analysis_2")
models_analysis_2[[model_name]] <- model

# Model4RT

model_acc <- brms::brm(
  probe_rt ~ 1 + cue_dimensionality + (1 + cue_dimensionality|subj_idx),
  data = data_behavior_full,
  family = gaussian(),
  iter = 2000,
  warmup = 1000,
...
ggplot2::ggplot(data = data_behavior_full, aes(x = cue_dimensionality, y = probe_rt)) +
  # 散点层
