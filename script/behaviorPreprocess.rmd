...
for (m in seq_along(model_list_analysis_2)){
  ...
  trace_par_cue2 <- as.data.frame(model)$b_cue_dimensionality2
  trace_par_cue3 <- as.data.frame(model)$b_cue_dimensionality3
  trace_par_cue4 <- as.data.frame(model)$b_cue_dimensionality4
  
  hdi_2 <- bayestestR::hdi(trace_par_cue2, ci = 0.95)
  rope_res_2 <- bayestestR::rope(trace_par_cue2, range = c(-0.1,0.1))
                                 
  hdi_3 <- bayestestR::hdi(trace_par_cue3, ci = 0.95)
  rope_res_3 <- bayestestR::rope(trace_par_cue3, range = c(-0.1,0.1))
  
  hdi_4 <- bayestestR::hdi(trace_par_cue4, ci = 0.95)
  rope_res_4 <- bayestestR::rope(trace_par_cue4, range = c(-0.1,0.1))
  
  p_2 <- plot(rope_res_2, rope_color = "grey70") +
    papaja::theme_apa() +
    ggtitle(paste(model_name, 'cue2'))
  
  p_3 <- plot(rope_res_3, rope_color = "grey70") +
    papaja::theme_apa() +
    ggtitle(paste(model_name, 'cue3'))
  
  p_4 <- plot(rope_res_4, rope_color = "grey70") +
    papaja::theme_apa() +
    ggtitle(paste(model_name, 'cue4'))
  
  plot_list <- c(plot_list, p_2, p_3, p_4)
}
